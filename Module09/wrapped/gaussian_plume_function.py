...
from Module09.wrapped.sigma_plume_calculation import calc_sigmas


def gauss_plume_func(Q, u, dir1, x, y, z, xs, ys, H, STABILITY):
    u1 = u
    ...
    (sig_y, sig_z) = calc_sigmas(STABILITY + 1, downwind)

    # 计算公式
    part_A = Q / (2 * np.pi * u1 * sig_y[ind] * sig_z[ind])
    part_B = (np.exp(-(z[ind] - H)**2 / (2 * sig_z[ind]**2)) + np.exp(-(z[ind] + H)**2 / (2 * sig_z[ind]**2)))
    part_C = np.exp(-crosswind[ind]**2 / (2 * sig_y[ind]**2))
    C[ind] = part_A * part_B * part_C
