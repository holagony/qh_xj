# -*- coding: utf-8 -*-
"""
Created on Mon Sep  1 15:21:11 2025
...
def re_frs_report(snow_result,year_df,data_dir):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    doc_path=os.path.join(cfg['report']['template'],'Module04','RE_FRS.docx')
    ...
    snow_pressure=dict(main_return_result['max_values_frs_prs'])
    ...
    dic['max_snow']=year_df['FRS_Depth_Max'].max()

    result_snow=pd.DataFrame(columns=['年份','冻土深度'])
    result_snow['年份']=year_df.index.year
    result_snow['冻土深度']=year_df.reset_index()['FRS_Depth_Max']
    ...
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'RE_FRS.docx')
    doc.save(report)
    ...
    creat_table(document,result_snow,'年）最大冻土深度（单位：cm）')
    creat_table(document,max_values,'期最大冻土深度（单位：cm）')
...
def re_frs_report_pg(snow_result,year_df,methods,data_dir):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    doc_path=os.path.join(cfg['report']['template'],'Module04','RE_FRS_P.docx')
    doc=DocxTemplate(doc_path)
