# -*- coding: utf-8 -*-
"""
Created on Wed May 29 15:07:48 2024
...
def hail_days_report(table1,table2,table3,month_data,data_dir,main_sta_ids):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    doc_path=os.path.join(cfg['report']['template'],'Module03','HAIL_DAYS.docx')
    ...
    average_pre_picture_hournum=plot_picture(table1u, '年份',station_name+'站','冰雹日数（d）','d','历年平均冰雹日数变化.png',10,10,data_dir)

    # plt.figure(figsize=(10, 6))
    # plt.bar(years,table1u[station_name+'站'], width=0.4, color='skyblue', label='冰雹日数')
    # plt.plot(years, slope * years + intercept, color='red', label='线性（冰雹日数）')

    # plt.grid(axis='y', linestyle='--', alpha=0.7)    
    # plt.xlabel('年')
    # plt.ylabel('冰雹日数（d）')
    # plt.xticks(years[::3])
    # plt.ylim(dic['average_pre_1'], dic['average_pre_2']+0.5)
    # plt.legend()
    
    # average_pre_picture_hournum=os.path.join(data_dir,'历年平均冰雹日数变化.png')
    ...
    average_pre_picture_month=plot_picture_2(months,table3u.iloc[1,2::],dic,'平均冰雹日数（d）','average_pre_m1','average_pre_m2','平均冰雹日数逐月变化.png',100,2,data_dir)
    ...
    # average_pre_picture_month=os.path.join(data_dir,'平均冰雹日数逐月变化.png')
    ...
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'HAIL_DAYS.docx')
    doc.save(report)
