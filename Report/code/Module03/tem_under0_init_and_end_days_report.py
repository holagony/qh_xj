# -*- coding: utf-8 -*-
"""
Created on Thu Jun  6 14:17:50 2024
...
def tem_under0_init_and_end_days_report(table4,table5,day_data,data_dir,main_sta_ids):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    doc_path=os.path.join(cfg['report']['template'],'Module03','tem_under0_init_and_end_days_report.docx')
    ...
    try:
        ...
        dic['max_jian_num_year1']=table5u.iloc[1,3].split('(')[1][0:4]
        dic['max_jian_num_year2']=table5u.iloc[1,3].split('(')[1][0:4]
    ...
    try:
        ...
        dic['min_jian_num_year1']=table5u.iloc[2,3].split('(')[1][0:4]
        dic['min_jian_num_year2']=table5u.iloc[2,3].split('(')[1][0:4]
    ...
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'tem_under0_init_and_end_days_report.docx')
    doc.save(report)
    ...
    # 填充表格数据
    creat_table(document,table4u,'平均最低气温小于等于0度日参数表')
    creat_table(document,table5u,'平均最低气温小于等于0度日参数统计表')
