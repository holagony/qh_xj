# -*- coding: utf-8 -*-
"""
Created on Wed Jun  5 17:07:48 2024
...
def drsnow_days_report(table1,table2,table3,month_data,data_dir,main_sta_ids):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    doc_path=os.path.join(cfg['report']['template'],'Module03','DRSNOW_DAYS.docx')
    ...
    average_pre_picture_hournum=plot_picture(table1u, '年份',station_name+'站','吹雪日数（d）','d','历年平均吹雪日数变化.png',10,10,data_dir)
    ...
    average_pre_picture_month=plot_picture_2(months,table3u.iloc[1,2::],dic,'平均吹雪日数（d）','average_pre_m1','average_pre_m2','平均吹雪日数逐月变化.png',100,2,data_dir)
    ...
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'DRSNOW_DAYS.docx')
    doc.save(report)
