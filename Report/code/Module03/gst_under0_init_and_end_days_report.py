# -*- coding: utf-8 -*-
"""
Created on Thu Jun  6 14:19:11 2024
...
def gst_under0_init_and_end_days_report(table4,table5,day_data,data_dir,main_sta_ids):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    doc_path=os.path.join(cfg['report']['template'],'Module03','gst_under0_init_and_end_days_report.docx')
    ...
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'gst_under0_init_and_end_days_report.docx')
    doc.save(report)
    ...
    # 填充表格数据
    creat_table(document,table4u,'平均地面最低气温小于等于0度日参数表')
    creat_table(document,table5u,'平均地面最低气温小于等于0度日参数统计表')
