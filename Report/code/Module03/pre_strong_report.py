# -*- coding: utf-8 -*-
"""
Created on Mon Jun 16 17:34:47 2025
...
def pre_strong_report(table1,table2,table3,month_data,data_dir,main_sta_ids):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    doc_path=os.path.join(cfg['report']['template'],'Module03','PRE_STRONG_DAYS.docx')
    ...
    ## 相对湿度
    # 年际变化
    dic['num_years']=len(table1)
    dic['station_name']=station_name
    dic['start_year']=table1u['年份'][0]
    dic['end_year']=table1u.iloc[-1,1]
    dic['sum_pre']=int(table2u.iloc[0,1])
    dic['average_pre']=table2u.iloc[0,2]

    
    dic['average_pre_1']=int(table2u.iloc[0,3])
    dic['average_pre_1_year']=table2u.iloc[0,4]
    dic['average_pre_2']=int(table2u.iloc[0,5])
    dic['average_pre_2_year']=table2u.iloc[0,6]
    ...
    average_pre_picture_hournum=plot_picture(table1u, '年份',station_name+'站','强降水日数（d）','d','历年平均强降水日数变化.png',20,20,data_dir)

    # plt.figure(figsize=(10, 6))
    # plt.bar(years,table1u[station_name+'站'], width=0.4, color='skyblue', label='强降水日数')
    # plt.plot(years, slope * years + intercept, color='red', label='强降水日数')

    # plt.grid(axis='y', linestyle='--', alpha=0.7)    
    # plt.xlabel('年')
    # plt.ylabel('强降水日数（d）')
    # plt.xticks(years[::3])
    # plt.ylim(dic['average_pre_2'], dic['average_pre_1']+10)
    # plt.legend()
    
    # average_pre_picture_hournum=os.path.join(data_dir,'历年平均强降水日数变化.png')
    ...
    average_pre_picture_month=plot_picture_2(months,table3u.iloc[1,2::],dic,'平均强降水日数（d）','average_pre_m1','average_pre_m2','平均强降水逐月变化.png',100,2,data_dir)
    ...
    # plt.grid(axis='y', linestyle='--', alpha=0.7)    
    # plt.xlabel('月')
    # plt.ylabel('平均强降水日数（d）')
    # plt.xticks(months)
    # plt.ylim(dic['average_pre_m1'], dic['average_pre_m2']+0.5)
    
    # average_pre_picture_month=os.path.join(data_dir,'平均降水量逐月变化.png')
    ...
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'PRE_STRONG_DAYS.docx')
    doc.save(report)
