# -*- coding: utf-8 -*-
"""
Created on Thu Jun  6 14:16:45 2024
...
def thund_init_and_end_days_report(table4,table5,day_data,data_dir,main_sta_ids):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    doc_path=os.path.join(cfg['report']['template'],'Module03','thund_init_and_end_days_report.docx')
    ...
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'thund_init_and_end_days_report.docx')
    doc.save(report)
    ...
    # 填充表格数据
    creat_table(document,table4u,'平均雷暴日参数表')
    creat_table(document,table5u,'平均雷暴日参数统计表')
