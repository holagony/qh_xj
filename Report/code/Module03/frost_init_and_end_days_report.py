...
def frost_init_and_end_days_report(table4,table5,day_data,data_dir,main_sta_ids):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    doc_path=os.path.join(cfg['report']['template'],'Module03','frost_init_and_end_days_report.docx')
    ...
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'frost_init_and_end_days_report.docx')
    doc.save(report)
    ...
    # 填充表格数据
    creat_table(document,table4u,'平均霜日参数表')
    creat_table(document,table5u,'平均霜日参数统计表')
