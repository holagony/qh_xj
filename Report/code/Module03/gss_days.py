# -*- coding: utf-8 -*-
"""
Created on Wed Jun  5 17:13:23 2024
...
def gss_days_report(table1,table2,table3,month_data,data_dir,main_sta_ids):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
        
    doc_path=os.path.join(cfg['report']['template'],'Module03','GSS_DAYS.docx')
    ...
    average_pre_picture_hournum=plot_picture(table1u, '年份',station_name+'站','积雪日数（d）','d','历年平均积雪日数变化.png',10,10,data_dir)

    # plt.figure(figsize=(10, 6))
    # plt.bar(years,table1u[station_name+'站'], width=0.4, color='skyblue', label='积雪日数')
    # plt.plot(years, slope * years + intercept, color='red', label='线性（积雪日数）')
    ...
    # average_pre_picture_hournum=os.path.join(data_dir,'历年平均积雪日数变化.png')
    ...
    average_pre_picture_month=plot_picture_2(months,table3u.iloc[1,2::],dic,'平均积雪日数（d）','average_pre_m1','average_pre_m2','平均积雪日数逐月变化.png',100,2,data_dir)
    ...
    # average_pre_picture_month=os.path.join(data_dir,'平均积雪日数逐月变化.png')
    ...
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'GSS_DAYS.docx')
    doc.save(report)
