...
def returnPre_report(years,sta_ids,daily_df,r,data_dir):
    ...
    dic['unit']='mm'
    # 数据
    max_data=pd.DataFrame(r['return_result']['max_values'])
    ...
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'RETURN_PRE.docx')
    doc.save(report)
