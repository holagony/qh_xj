...
def tem_report(years,sta_ids,sub_sta_ids,daily_df,wind_dict,data_dir):
    ...
    #%% Part 1
    dic['element_title']='极端气温'
    dic['element_subtitle_1']='极端最高气温'
    ...
    dic['unit']='℃'
    # 年数据
    wind_max_year_main=wind_dict['历年最高气温'][['年份',sta_ids]]
    ...
    # 月数据
    win_max_month_main=wind_dict['累年各月平均最高气温'][wind_dict['累年各月平均最高气温']['站号']==sta_ids].iloc[:,1::]
    dic['max_month_1'] = win_max_month_main.idxmax(axis=1).iloc[0][0]
    dic['max_month_data_1'] = win_max_month_main.max(axis=1).iloc[0]
    # 图形绘制
    average_win_picture_year_1=plot_picture(wind_max_year_main, '年份',sta_ids,'极端最高气温(℃)','（℃）','极端最高气温逐年变化图.png',0.3,0.3,data_dir,R= round(float(r), 3))
    months = list(range(1, win_max_month_main.shape[1] + 1))
    max_win_picture_month_1 = plot_picture_2(months,win_max_month_main.iloc[0].values,dic,'极端最高气温(℃)',0,0,'极端最高气温逐月变化图.png',0,0.5,data_dir,)
    ...
    #%% Part 3
    dic['element_subtitle_2']='极端最低气温'
    # 年数据
    wind_inst_year_main=wind_dict['历年最低气温'][['年份',sta_ids]]
    ...
    # 月数据
    win_max_month_main=wind_dict['累年各月平均最低气温'][wind_dict['累年各月平均最低气温']['站号']==sta_ids].iloc[:,1::]
    dic['max_month_2'] = win_max_month_main.idxmax(axis=1).iloc[0][0]
    dic['max_month_data_2'] = win_max_month_main.max(axis=1).iloc[0]
    # 图形绘制
    average_win_picture_year_2=plot_picture(wind_inst_year_main, '年份',sta_ids,'极端最低气温(℃)','（℃）','极端最低气温逐年变化图.png',0.3,0.3,data_dir,R= round(float(r), 3))
    months = list(range(1, win_max_month_main.shape[1] + 1))
    max_win_picture_month_2 = plot_picture_2(months,win_max_month_main.iloc[0].values,dic,'极端最低气温(℃)',0,0,'极端最低气温逐月变化图.png',0,0.5,data_dir,)
    ...
    if sub_sta_ids is not None:
        ...
        #%% Part 2
        wind_max_year_sub=wind_dict['历年最高气温']
        ...
        if _down_n == 0 and _up_n > 0:
            dic['sub_explain_1'] = "各气象站极端最高气温趋势均呈上升趋势。"
        elif _up_n == 0 and _down_n > 0:
            dic['sub_explain_1'] = "各气象站极端最高气温趋势均呈下降趋势。"
        else:
            dic['sub_explain_1'] = (
                f"各气象站极端最高气温趋势中，{'、'.join(_up_list)}气象站呈上升趋势，"
        ...
        _ax.set_ylabel('极端最高气温(℃)', fontsize=14)
        _ax.set_xticks(_x)
        ...
        _sub_year_bar_path = os.path.join(data_dir, '各站极端最高气温逐年对比柱状图.png')
        _fig.tight_layout()
        ...
        win_max_month_sub=wind_dict['累年各月平均最高气温']
        
        #%% Part4
        
        wind_inst_year_sub=wind_dict['历年最低气温']
        ...
        _down_n = len(_down_list)
        if _down_n == 0 and _up_n > 0:
            dic['sub_explain_2'] = "各气象极端最低气温趋势均呈上升趋势。"
        elif _up_n == 0 and _down_n > 0:
            dic['sub_explain_2'] = "各气象站极端最低气温趋势均呈下降趋势。"
        else:
            dic['sub_explain_2'] = (
                f"各气象站极端最低气温趋势中，{'、'.join(_up_list)}气象站呈上升趋势，"
                f"下降站点为{'、'.join(_down_list)}气象站呈下降趋势。"
            )
        ...
        for _i, _c in enumerate(_station_cols):
            ...
            _ax.bar(_x + (_i - (_n - 1) / 2) * _width, _y, width=_width, label=_label)
        _ax.set_xlabel('年', fontsize=14)
        _ax.set_ylabel('极端最低气温(℃)', fontsize=14)
        _ax.set_xticks(_x)
        _ax.set_xticklabels(_bar_years, rotation=90 if len(_bar_years) > 15 else 0, fontsize=12)
        _ax.tick_params(axis='y', labelsize=12)
        _ax.grid(axis='y', linestyle='--', alpha=0.3, color='gray')
        _ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.12), ncol=min(6, _n), frameon=False)
        _sub_year_bar_path = os.path.join(data_dir, '各站极端最低气温逐年对比柱状图.png')
        _fig.tight_layout()
        _fig.savefig(_sub_year_bar_path, bbox_inches='tight', dpi=200)
        plt.close(_fig)
        dic['average_picture_m_2'] = InlineImage(doc, _sub_year_bar_path, width=Mm(130))
    
        win_inst_month_sub=wind_dict['累年各月平均最低气温']
