# -*- coding: utf-8 -*-
"""
Created on Mon Feb  2 10:13:27 2026
...
def returnTem_report(years,sta_ids,daily_df,r,data_dir):

    if not os.path.exists(data_dir):
        os.makedirs(data_dir)
      
    doc_path=os.path.join(cfg['report']['template'],'Module14','Return_all.docx')
    ...
    #%% Part 1
    dic['element_title']='极端气温极值推算'
    dic['element_subtitle_1']='极端最高气温极值推算'
    ...
    dic['unit']='℃'
    # 数据
    max_data=pd.DataFrame(r['main_return_result']['max_values_max'])
    ...
    #%% Part 2
    dic['element_subtitle_2']='极端最低气温极值推算'
    inst_data=pd.DataFrame(r['main_return_result']['max_values_min'])
    column_inst = inst_data.max(axis=1)
    inst_data.insert(0, '重现期', ['30年一遇','50年一遇','100年一遇'])    
    inst_data.columns=['重现期', '极值I型', '皮尔逊Ⅲ型']
    
    dic['data_30_2']=column_inst[0]
    dic['data_50_2']=column_inst[1]
    dic['data_100_2']=column_inst[2]
    
    # 模版文件读取写入字典
    doc.render(dic)
    # 保存结果到新的docx文件
    report=os.path.join(data_dir,'RETURN_TEM.docx')
    doc.save(report)
    ...
    creat_table(document,max_data,f"气象站各重现期{dic['element_subtitle_2']}估计（{dic['unit']}）")
