...
def snow_report(years,sta_ids,sub_sta_ids,daily_df,wind_dict,data_dir):
    ...
    dic['unit']='mm'
    # 年数据
    wind_max_year_main=wind_dict['历年最大积雪深度'][['年份',sta_ids]]
    ...
    # 月数据
    win_max_month_main=wind_dict['累年各月平均积雪深度'][wind_dict['累年各月平均积雪深度']['站号']==sta_ids].iloc[:,1::]
    dic['max_month_1'] = win_max_month_main.idxmax(axis=1).iloc[0][0]
    dic['max_month_data_1'] = win_max_month_main.max(axis=1).iloc[0]
    # 图形绘制
    average_win_picture_year_1=plot_picture(wind_max_year_main, '年份',sta_ids,'极端最大日积雪深度(mm)','（mm）','极端最大日积雪深度逐年变化图.png',0.3,0.3,data_dir,R= round(float(r), 3))
    months = list(range(1, win_max_month_main.shape[1] + 1))
    max_win_picture_month_1 = plot_picture_2(months,win_max_month_main.iloc[0].values,dic,'极端最大日积雪深度(mm)',0,0,'极端最大日积雪深度逐月变化图.png',0,0.5,data_dir,)
    ...
    if sub_sta_ids is not None:
        ...
        #%% Part 2
        wind_max_year_sub=wind_dict['历年最大积雪深度']
        ...
        if _down_n == 0 and _up_n > 0:
            dic['sub_explain_1'] = "各气象站极端最大日积雪深度趋势均呈上升趋势。"
        elif _up_n == 0 and _down_n > 0:
            dic['sub_explain_1'] = "各气象站极端最大日积雪深度趋势均呈下降趋势。"
        else:
            dic['sub_explain_1'] = (
                f"各气象站极端最大日积雪深度趋势中，{'、'.join(_up_list)}气象站呈上升趋势，"
        ...
        _ax.set_ylabel('极端最大日积雪深度(mm)', fontsize=14)
        _ax.set_xticks(_x)
        ...
        _sub_year_bar_path = os.path.join(data_dir, '各站极端最大日积雪深度逐年对比柱状图.png')
        _fig.tight_layout()
        ...
        win_max_month_sub=wind_dict['累年各月平均积雪深度']
