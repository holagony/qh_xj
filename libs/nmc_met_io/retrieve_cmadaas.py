...
def get_rest_result(interface_id, params, url_only=False, dns=None, port=None, data_format='json'):
    """
...
def cmadaas_obs_convert_type(obs_data):
    ...
    for column in obs_data.columns:
        if column.upper().startswith('STATION'):
            continue
        if column.lower() in ["province", "country", "city", "cnty", "town", "data_id", "rep_corr_id", "admin_code_chn"]:
            continue
...
def cmadaas_get_obs_latest_time(data_code="SURF_CHN_MUL_HOR", latestTime=12):
    ...
    params = {'dataCode': data_code, 'latestTime': str(latestTime + 8)}  # 由于服务器是世界时, 因此需要+8才能与当前时间比较.
...
def cmadaas_get_obs_files(times, data_code="SURF_CMPA_RT_NC", out_dir=None, pbar=True):
    ...
    if times[0] == '[':
        # set retrieve parameters
        params = {'dataCode': data_code, 'timeRange': times}
        interface_id = "getSurfFileByTimeRange"
    else:
        # set retrieve parameters
        params = {'dataCode': data_code, 'times': times}
...
def cmadaas_obs_by_time(times,
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'times': times, 'orderby': order if order is not None else "Datetime:ASC", 'elements': elements}
...
def cmadaas_obs_by_time_range(time_range,
                              data_code="SURF_CHN_MUL_HOR_N",
                              sta_levels=None,
                              ranges=None,
                              order=None,
...
def cmadaas_obs_by_time_and_id(times, data_code="SURF_CHN_MUL_HOR_N", sta_levels=None, ranges=None, order=None, count=None, trans_type=True, elements="Station_Id_C,Datetime,TEM", sta_ids="54511"):
    """
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements, 'times': times, 'staIds': sta_ids, 'orderby': order if order is not None else "Datetime:ASC"}
    if sta_levels is not None: params['staLevels'] = sta_levels
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_obs_by_time_range_and_id(time_range,
                                     data_code="SURF_CHN_MUL_HOR_N",
                                     sta_levels=None,
                                     ranges=None,
                                     order=None,
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements, 'timeRange': time_range, 'staIds': sta_ids, 'orderby': order if order is not None else "Datetime:ASC"}
    if sta_levels is not None: params['staLevels'] = sta_levels
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_obs_by_time_range_and_id_radi(time_range,
                                          data_code="SURF_CHN_MUL_HOR_N",
                                          sta_levels=None,
                                          ranges=None,
                                          order=None,
...
def cmadaas_obs_in_rect_by_time(times, limit, data_code="SURF_CHN_MUL_HOR_N", sta_levels=None, ranges=None, order=None, count=None, trans_type=True, elements="Station_Id_C,Datetime,Lat,Lon,TEM"):
    """
...
def cmadaas_obs_in_rect_by_time_range(time_range, limit, data_code="SURF_CHN_MUL_HOR_N", ranges=None, order=None, count=None, trans_type=True, elements="Station_Id_C,Datetime,Lat,Lon,TEM"):
    """
...
def cmadaas_obs_in_admin_by_time(times,
                                 admin="110000",
                                 data_code="SURF_CHN_MUL_HOR_N",
                                 sta_levels=None,
                                 ranges=None,
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements, 'times': times, 'adminCodes': admin, 'orderby': order if order is not None else "Datetime:ASC"}
    if sta_levels is not None: params['staLevels'] = sta_levels
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_obs_in_admin_by_time_range(time_range,
                                       admin="110000",
                                       data_code="SURF_CHN_MUL_HOR_N",
                                       sta_levels=None,
                                       ranges=None,
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements, 'timeRange': time_range, 'adminCodes': admin, 'orderby': order if order is not None else "Datetime:ASC"}
    if sta_levels is not None: params['staLevels'] = sta_levels
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_obs_in_basin_by_time(times,
                                 basin="CJLY",
                                 data_code="SURF_CHN_MUL_HOR_N",
                                 sta_levels=None,
                                 ranges=None,
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements, 'times': times, 'basinCodes': basin, 'orderby': order if order is not None else "Datetime:ASC"}
    if sta_levels is not None: params['staLevels'] = sta_levels
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_obs_in_basin_by_time_range(time_range,
                                       basin="CJLY",
                                       data_code="SURF_CHN_MUL_HOR_N",
                                       sta_levels=None,
                                       ranges=None,
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements, 'timeRange': time_range, 'basinCodes': basin, 'orderby': order if order is not None else "Datetime:ASC"}
    if sta_levels is not None: params['staLevels'] = sta_levels
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_obs_by_period(minYear,
                          maxYear,
                          minMD,
                          maxMD,
                          data_code="SURF_CHN_MUL_HOR_N",
...
def cmadaas_obs_by_period_and_id(minYear,
                                 maxYear,
                                 minMD,
                                 maxMD,
                                 data_code="SURF_CHN_MUL_HOR_N",
...
def cmadaas_obs_in_admin_by_period(minYear,
                                   maxYear,
                                   minMD,
                                   maxMD,
                                   admin="110000",
...
def cmadaas_obs_grid_by_time(time_str, limit=None, data_code="SURF_CMPA_FAST_5KM", fcst_ele="PRE", zoom=None, units=None, scale_off=None, cache=True, cache_clear=True):
    """
    ...
    # set retrieve parameters
    if limit is None:
        params = {'dataCode': data_code, 'time': time_str, 'fcstEle': fcst_ele}
    ...
    lon_coord = ('lon', lon, {'long_name': 'longitude', 'units': 'degrees_east', '_CoordinateAxisType': 'Lon', 'axis': "X"})
    lat_coord = ('lat', lat, {'long_name': 'latitude', 'units': 'degrees_north', '_CoordinateAxisType': 'Lat', 'axis': "Y"})
    varname = fcst_ele
    ...
    data = xr.Dataset({varname: (['time', 'lat', 'lon'], data, varattrs)}, coords={'time': time_coord, 'lat': lat_coord, 'lon': lon_coord})
...
def cmadaas_obs_by_days_of_year(data_code="SURF_CHN_DAY_MMUT_19812010",
                                days_of_year=None,
                                elements="Station_Name,Station_Id_C,Day_Seq,TEM_Avg",
                                ranges=None,
                                count=None,
                                trans_type=True,
                                sta_ids="54511"):
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements, 'daysOfYear': days_of_year}
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_obs_by_pens_of_year(data_code="SURF_CHN_PEN_MMUT_19812010",
                                pens_of_year=None,
                                elements="Station_Name,Station_Id_C,Pen_Seq,TEM_Avg",
                                ranges=None,
                                count=None,
                                trans_type=True,
                                sta_ids="54511"):
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements, 'pensOfYear': pens_of_year}
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_obs_by_tens_of_year(data_code="SURF_CHN_TEN_MMUT_19812010",
                                tens_of_year=None,
                                elements="Station_Name,Station_Id_C,Ten_Seq,TEM_Avg",
                                ranges=None,
                                count=None,
                                trans_type=True,
                                sta_ids="54511"):
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements, 'tensOfYear': tens_of_year}
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_obs_by_months_of_year(data_code="SURF_CHN_MON_MMUT_19812010",
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements, 'monsOfYear': mons_of_year}
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_obs_by_years(data_code="SURF_CHN_YER_MMUT_19812010", elements="Station_Name,Station_Id_C,RHU_Avg", ranges=None, count=None, trans_type=True, sta_ids=None):
    """
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'elements': elements}
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
...
def cmadaas_sounding_by_time(times,
                             data_code="UPAR_CHN_MUL_FTM",
                             ranges=None,
                             order=None,
                             count=None,
                             distinct=False,
...
def cmadaas_sounding_by_time_and_press(times,
                                       data_code="UPAR_CHN_MUL_FTM",
                                       press=500,
                                       ranges=None,
                                       order=None,
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'times': times, 'pLayers': str(press).strip(), 'orderby': order if order is not None else "Datetime:ASC", 'elements': elements}
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
    if distinct: params['distinct'] = "true"
...
def cmadaas_sounding_by_time_and_height(times,
                                        data_code="UPAR_CHN_MUL_FTM",
                                        height=5000,
                                        ranges=None,
                                        order=None,
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'times': times, 'hLayers': str(height).strip(), 'orderby': order if order is not None else "Datetime:ASC", 'elements': elements}
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
    if distinct: params['distinct'] = "true"
...
def cmadaas_sounding_in_rect_by_time(times,
                                     limit=[3.51, 73.33, 53.33, 135.05],
                                     data_code="UPAR_CHN_MUL_FTM",
                                     ranges=None,
                                     order=None,
                                     count=None,
...
def cmadaas_sounding_by_time_and_id(times,
                                    data_code="UPAR_CHN_MUL_FTM",
                                    sta_ids='54511',
                                    ranges=None,
                                    order=None,
    ...
    # set retrieve parameters
    params = {'dataCode': data_code, 'times': times, 'staIds': str(sta_ids).strip(), 'orderby': order if order is not None else "Datetime:ASC", 'elements': elements}
    if ranges is not None: params['eleValueRanges'] = ranges
    if count is not None: params['limitCnt'] = str(count)
    if distinct: params['distinct'] = "true"
...
def cmadaas_sounding_in_rect_by_time_range(time_range,
                                           limit=[3.51, 73.33, 53.33, 135.05],
                                           data_code="UPAR_CHN_MUL_FTM",
                                           ranges=None,
                                           order=None,
...
def cmadaas_analysis_by_time(time_str,
                             limit=None,
                             data_code='NAFP_CLDAS2.0_NRT_ASI_NC',
                             levattrs={
    ...
    # set retrieve parameters
    if limit is None:
        params = {'dataCode': data_code, 'time': time_str, 'levelType': str(level_type).strip(), 'fcstEle': fcst_ele}
        if zoom is not None: params['zoomOut'] = str(zoom)
    ...
    if fcst_level is None:
        ...
        data = xr.Dataset({varname: (['time', 'level', 'lat', 'lon'], data, varattrs)}, coords={'time': time_coord, 'level': level_coord, 'lat': lat_coord, 'lon': lon_coord})
...
def cmadaas_model_grid(data_code,
                       init_time,
                       valid_time,
                       fcst_ele,
                       fcst_level,
    ...
    # define coordinates and variables
    time_coord = ('time', time)
    lon_coord = ('lon', lon, {'long_name': 'longitude', 'units': 'degrees_east', '_CoordinateAxisType': 'Lon'})
    lat_coord = ('lat', lat, {'long_name': 'latitude', 'units': 'degrees_north', '_CoordinateAxisType': 'Lat'})
    if fcst_level != 0:
        level_coord = ('level', np.array([fcst_level]), levattrs)
    ...
    data.coords['forecast_period'] = ('time', fhour, {'long_name': 'forecast_period', 'units': 'hour'})
...
def cmadaas_model_grids(data_code, init_time, valid_times, fcst_ele, fcst_level, level_type, allExists=True, pbar=False, **kargs):
    """
...
def cmadaas_model_by_pionts(init_time_str, data_code='NAFP_FOR_FTM_HIGH_EC_ANEA', fcst_level=850, level_type=100, time_range=[0, 72], points="39.90/116.40", fcst_ele="TEM"):
    """
...
def cmadaas_model_by_piont_levels(init_time, fcst_levels=[1000, 925, 850, 700, 500], pbar=True, **kargs):
    """
    Retrieve grid point data from CMADaaS service.
    ...
    data = xr.DataArray(data, coords=[data.index.values, data.columns.values], dims=['time', 'level'], name=kargs['fcst_ele'])
...
def cmadaas_get_model_file(time, data_code="NAFP_FOR_FTM_HIGH_EC_ANEA", fcst_ele=None, level_type='100', out_dir=None, pbar=False, just_url=False):
    """
    ...
    if fcst_ele is None:
        ...
        params = {'dataCode': data_code, 'fcstEle': fcst_ele.strip(), 'levelType': str(level_type).strip()}
        if time[0] == '[':
            # set retrieve parameters
            params['timeRange'] = time
...
def cmadaas_get_rise_model_file(time, data_code="NAFP_WOG_ANA_100M", fcst_ele=None, userId=None, pwd=None, out_dir=None, pbar=False, just_url=False):
    ...
    if fcst_ele is None:
        ...
        params = {'dataCode': data_code, 'fcstEle': fcst_ele.strip(), 'userId': userId, 'pwd': pwd}
        if time[0] == '[':
            # set retrieve parameters
            params['timeRange'] = time
            interface_id = "getNafpFileByElementAndTimeRange"
            ...
            params['time'] = time
            interface_id = "getNafpFileByElementAndTime"
    params = {'dataCode': data_code, 'userId': userId, 'pwd': pwd}
    params['time'] = time
    interface_id = "getNafpFileByTime"
...
def get_rise_rest_result(interface_id, params, url_only=False, dns=None, port=None, data_format='json'):
...
def rise5_model_by_pionts(init_time_str, data_code='RMAPSRISE5', time_range=[0, 24], points="39.90/116.40", fcst_ele="2T,apcp_1hr,RH,10U,10V,10FG1"):
...
def rise_model_by_pionts(init_time_str, data_code='RMAPSRISE', time_range=[0, 24], points="39.90/116.40", fcst_ele="2T,RH,10U,10V,10FG1"):
