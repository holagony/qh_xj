...
module00 = Blueprint('module00', __name__)


@module00.route('/v1/check', methods=['POST'])
def check():
    '''
    数据缺测统计
    '''
    ...
    # todo 组装参数、下载数据
    result = celery_submit.delay("workerCheck", json_str)
    return jsonify({'code': 202, 'msg': '任务提交成功，开始计算...', 'data': {'task_id': result.id}})


@module00.route('/v1/p3', methods=['POST'])
def p3p3():
    json_str = request.get_data(as_text=True)  # 获取JSON字符串
    code = 200
    msg = '获取数据成功'
