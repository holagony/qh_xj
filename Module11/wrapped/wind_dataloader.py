...
def wind_tower_upload(paths, sta_id, lon, lat):
    ...
    lat: 定义的纬度
    '''
    total_path = glob.glob(os.path.join(paths, '*.xlsx'))
    df_all = []
    ...
    df_all['日期时间'] = '20' + df_all['日期时间']
    df_all['日期时间'] = pd.to_datetime(df_all['日期时间'], format='%Y-%m-%d-%H')
    df_all.set_index('日期时间', inplace=True)

    for col in df_all.columns:
        if ('时间' or '风向' or 'ID') in col:
            df_all[col] = df_all[col].apply(lambda x: np.nan if x in ['-', '--', '---', '----', np.nan] else int(x))
        else:
            df_all[col] = df_all[col].apply(lambda x: np.nan if x in ['-', '--', '---', '----', np.nan] else float(x))

    # 删除无用的列
    df_trans = df_all.copy()
    df_trans.drop(columns=['电压', 'ID'], inplace=True)
    df_trans = df_trans[df_trans.columns.drop(list(df_trans.filter(like='时间')) + list(df_trans.filter(like='温度')) + list(df_trans.filter(like='湿度')) + list(df_trans.filter(like='气压')))]
    new_cols = [col[-1] + '-' + col[:-1] for col in df_trans.columns]
    ...
    df_trans = df_trans.set_index(['datetime', '数值'])['高度层'].str.split('-', expand=True).reset_index()
    df_trans = df_trans.set_index(['datetime', 0, 1]).unstack()
    df_trans.columns = df_trans.columns.droplevel(0)
    df_trans = df_trans.rename_axis(columns=None).reset_index().rename(columns={0: '高度层'})
