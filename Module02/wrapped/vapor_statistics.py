...
def basic_vapor_statistics(vapor_day, data_dir):
    '''
    水汽压要素，历年和累年各月统计，
    使用天擎上的日数据，要素名称为天擎上默认的名称
    日值要素：['Station_Id_C', 'Station_Name', 'Year', 'Mon', 'Day', 'VAP_Avg']
    return: dataframe
    '''

    try:
        vapor = vapor_day['VAP_Avg'].to_frame()
        vapor.dropna(axis=0, how='any', inplace=True)

        # A.历年统计
        vapor_yearly = vapor.resample('1A', closed='right', label='right').mean()
        vapor_yearly['year'] = vapor_yearly.index.year
        vapor_yearly = vapor_yearly[['year', 'VAP_Avg']]
        vapor_yearly.reset_index(drop=True, inplace=True)

        year_max_idx = vapor.groupby([vapor.index.year], as_index=False)['VAP_Avg'].idxmax()
        max_idx = year_max_idx['VAP_Avg'].tolist()
        vapor_max = vapor.loc[max_idx]
        vapor_max['date'] = vapor_max.index.strftime("%m月%d日")
        vapor_max.reset_index(drop=True, inplace=True)

        year_min_idx = vapor.groupby([vapor.index.year], as_index=False)['VAP_Avg'].idxmin()
        min_idx = year_min_idx['VAP_Avg'].tolist()
        ...
        basic_vapor_yearly.columns = ['年份', '平均水汽压(hPa)', '最大水汽压(hPa)', '最大水汽压出现日期', '最小水汽压(hPa)', '最小水汽压出现日期']
        ...
        try:
            ...
            vapor_max_idx = vapor.groupby([vapor.index.year, vapor.index.month], as_index=False)['VAP_Avg'].idxmax()
            max_idx = vapor_max_idx['VAP_Avg'].tolist()
            ...
            vapor_min_idx = vapor.groupby([vapor.index.year, vapor.index.month], as_index=False)['VAP_Avg'].idxmin()
            min_idx = vapor_min_idx['VAP_Avg'].tolist()
            vapor_min = vapor.loc[min_idx]
            ...
            for i in range(1, 13):
                ...
                month_i_max = month_i_max[month_i_max['VAP_Avg'] == month_i_max['VAP_Avg'].max()]
                ...
                month_i_min = month_i_min[month_i_min['VAP_Avg'] == month_i_min['VAP_Avg'].min()]

                # 针对多个时间点数值相同的情况
                if len(month_i_min) > 1:
                    vapor_data = month_i_min.iloc[0, 0]  # columns: ['VAP_Avg', 'date', 'Year', 'Mon']
            ...
            mean_vapor_accum.index = ['平均水汽压(hPa)']
            ...
            max_vapor_accum['VAP_Avg'] = max_vapor_accum['VAP_Avg'].astype(float)
            max_row = max_vapor_accum[max_vapor_accum['VAP_Avg'] == max_vapor_accum['VAP_Avg'].max()].reset_index(drop=True)

            if len(max_row) == 1:
                vapor = max_row.loc[0, 'VAP_Avg']
            ...
            max_vapor_accum.index = ['最大水汽压(hPa)', '最大水汽压出现日期', '最大水汽压出现年份']
            max_vapor_accum['全年'] = values_list

            # min
            min_vapor_accum = pd.concat(min_vapor_accum, axis=0, ignore_index=True)
            min_vapor_accum['VAP_Avg'] = min_vapor_accum['VAP_Avg'].astype(float)
            min_row = min_vapor_accum[min_vapor_accum['VAP_Avg'] == min_vapor_accum['VAP_Avg'].min()].reset_index(drop=True)

            if len(min_row) == 1:
                vapor = min_row.loc[0, 'VAP_Avg']
            ...
            min_vapor_accum.index = ['最小水汽压(hPa)', '最小水汽压出现日期', '最小水汽压出现年份']
