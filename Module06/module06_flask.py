...
from Module06.module06_handler import climate_assessment_handler

module06 = Blueprint('module06', __name__)


@module06.route('/v1/climate_assessment', methods=['POST'])
def climate_assessment():
    ...
    if is_async == 1 or is_async is True or is_async == '1':
        result = celery_submit.delay('workerClimateAssessment', json_str)
        return jsonify({'code': 202, 'msg': '任务提交成功，开始计算...', 'data': {'task_id': result.id}})

    result_dict = climate_assessment_handler(data_json)
